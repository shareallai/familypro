...
### 2.2 Task-specific skills or workflows
...
## 3. Codex: `AGENTS.md` is the main layer, skills are demand-loaded
...
### 3.1 `AGENTS.md` is part of startup context
...
### 3.2 Skills are not fully loaded up front
...
### 3.3 Seeing `AGENT.md` in the wild does not mean the official name changed
...
## 4. Claude Code: `CLAUDE.md` is the project memory entry point, with rules and skills around it
...
### 4.1 `CLAUDE.md` is the persistent project instruction file
...
### 4.2 `.claude/rules/` is for modular project rules
...
### 4.3 `.claude/skills` is the modern skill directory; old `.claude/commands` still exists
...
### 4.4 `CLAUDE.md` plays a similar role to Codex `AGENTS.md`, but not at the same instruction level
...
## 5. Cursor: think "Rules first," not "just another AGENTS.md system"
...
### 5.1 `.cursor/rules/*.mdc` is Cursor's native project rule format
...
### 5.2 Default project behavior in Cursor still centers on Rules
...
### 5.3 Cursor now explicitly recognizes `AGENTS.md` too
...
## 6. The four things people mix up most often

### 6.1 A skill is not your main project instruction file
...
### 6.2 Similar filenames do not imply similar loading behavior
...
### 6.3 Claude Code is especially sensitive to where you launch it
...
### 6.4 Real repos often contain custom filenames that are not official standards
...
## 7. If you need a practical migration map, use three layers
...
### 7.1 Layer one: shared repo defaults
...
### 7.2 Layer two: task-specific SOPs
...
### 7.3 Layer three: personal preferences
