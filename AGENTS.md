...
- 写作语气必须面向“想了解当前博客主题并准备据此做判断或上手实践”的读者，优先回答读者最关心的问题，避免公告腔、术语堆砌和模板化复述。
...
- 章节标题默认使用结构化序号：`h2` 使用 `1.`、`2.`、`3.`；`h3` 使用 `1.1`、`1.2`。同层级编号必须连续，且编号必须跟随真实结构而不是装饰。
- 参考资料章节标题属于固定校验例外：中文必须使用 `## 官方参考`，非中文必须使用 `## References`，该标题不加序号。
...
- 仓库内博客插图与截图资源工作流的 source of truth 位于 `.agents/skills/familypro-blog-illustration-workflow/`。
- 当任务涉及为博客新增、替换、整理、引用或验证插图/截图/图片资源时，必须先阅读 `.agents/skills/familypro-blog-illustration-workflow/SKILL.md`。该类任务应优先使用用户提供的原始图片，文章专属图片默认放在 `public/blog/{slug}/`，不得擅自转成 SVG 或生成替代图，除非用户明确要求。
